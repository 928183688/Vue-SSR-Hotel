      ...
      <div>
        <div v-for="(item,index) in data.insurances" :key="index" class="insurance-item">
          <el-checkbox
            ...
            border
            @change="handleChangeClick(item.id)"
          />
        </div>
      </div>
    </div>
...
export default {
  ...
  computed: {
    allPrice() {
      ...
      // 默认显示价格
      pic += this.data.seat_infos.org_settle_price * len
      // 油价
      pic += this.data.airport_tax_audlet * len
      // 保险价格
      this.insurances.forEach((v) => {
        pic += this.data.insurances[v - 1].price * len
      })
      this.$emit('setAllPrice', pic)
      return pic
    }
  },
  methods: {
