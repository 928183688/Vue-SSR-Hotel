    ...
    >
      ...
      <div class="post-content">
        <h4 :title="data.title" class="post-title">
          <nuxt-link :to="`/post/detail?id=${data.id}`" v-html="data.title ">
            {{ data.title }}
          </nuxt-link>
        </h4>
        <p class="post-desc">
          <nuxt-link :to="`/post/detail?id=${data.id}`" v-html="data.summary " />
        </p>

        <el-row type="flex" justify="space-between" class="post-info">
          <el-row type="flex" align="middle" class="post-info-left">
            <span><i class="el-icon-location-outline" />{{ data.city.name }}</span>
            ...
            <span><i class="el-icon-view" /> {{ data.watch || 0 }}</span>
          </el-row>
    ...
    <div v-if="showType === `card`" class="post-item card">
      ...
      <el-row type="flex" justify="space-between" class="post-info">
        <el-row type="flex" align="middle" class="post-info-left">
          ...
          <span>
            <i class="el-icon-view" /> {{ data.watch || 0 }}
          </span>
        </el-row>
